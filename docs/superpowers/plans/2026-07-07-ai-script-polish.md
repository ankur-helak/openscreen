...
- **Deviation (accepted 2026-07-08, user decision):** spec §9 / Decision 7 called for a first-use consent *enforced in the main process* (with a "don't show again") before the first OpenAI call. As built, consent is a static privacy note (`polish.keyDialog.privacyNote`) rendered in the key dialog — which appears before the key is entered/saved, so disclosure-before-first-call holds in practice. The main-process enforcement was **not** implemented; the whole-branch review flagged it and the user accepted it as a documented deviation for v1. A minimal main-process consent flag can be added later if required.
